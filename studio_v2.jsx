...
// ═══════════════════════════════════════════════════════════════
//  PREMIERE PIPELINE — PNG sequence rendering + manifest
// ═══════════════════════════════════════════════════════════════
const PREMIERE_FPS=25;

/**
 * Render a graphic animation as individual PNG frame blobs.
 * Returns an array of Blobs — one per frame.
 * Animation runs for ~0.8s then holds at final state.
 */
async function renderPNGSequence(g, brand, ratio, durationSec, fps=PREMIERE_FPS) {
  const AR=RATIOS[ratio||"16:9"]||RATIOS["16:9"];
  const cvs=document.createElement("canvas"); cvs.width=AR.W; cvs.height=AR.H;
  const totalFrames=Math.ceil(durationSec*fps);
  const animFrames=Math.round(0.8*fps); // 0.8s entrance animation
  const blobs=[];
  for(let f=0;f<totalFrames;f++){
    const progress=clamp(f/(animFrames*0.9),0,1);
    drawGraphic(cvs,g,brand,ratio,progress);
...
/**
 * Render a title card animation as PNG frames.
 */
async function renderTitleCardSequence(brand, ratio, durationSec, fps=PREMIERE_FPS) {
  const AR=RATIOS[ratio||"16:9"]||RATIOS["16:9"];
  const cvs=document.createElement("canvas"); cvs.width=AR.W; cvs.height=AR.H;
  const totalFrames=Math.ceil(durationSec*fps);
  const animFrames=Math.round(0.8*fps);
  const blobs=[];
  for(let f=0;f<totalFrames;f++){
    const progress=clamp(f/(animFrames*0.9),0,1);
    drawTitleCard(cvs,brand,ratio,progress);
    const blob=await new Promise(resolve=>cvs.toBlob(resolve,"image/png"));
    blobs.push(blob);
  }
  return blobs;
...
/**
 * Convert HH:MM:SS timestamp to frame number at given fps.
 */
function timestampToFrame(ts, fps=PREMIERE_FPS) {
  if(!ts) return 0;
...
/**
 * Generate a Premiere manifest JSON from project data.
 */
function generatePremiereManifest(project, brand, selectedGfx, ratio, fps=PREMIERE_FPS) {
  const AR=RATIOS[ratio||"16:9"]||RATIOS["16:9"];
  const ratioKey=ratio.replace(":","x");
  const pn=project.name.replace(/[^a-zA-Z0-9]+/g,"_");

  const items=selectedGfx.map((g,i)=>{
    const folderName=`${pn}_${ratioKey}_${String(i+1).padStart(2,"0")}_${g.label||g.template}`;
    const durSec=g.duration||2;
    return {
  ...
  return {
    projectName:`${project.name} ${ratioKey}`,
    fps,
...
/**
 * Full Premiere package export:
 * - One folder per graphic containing frame_0001.png … frame_NNNN.png
 * - manifest.json for the Premiere import script
 *
 * Requires JSZip (loaded from CDN).
 * Returns a Blob (zip file).
 */
async function exportPremierePackage(project, brand, selectedGfx, ratio, fps, onProgress) {
  if(typeof JSZip==="undefined") throw new Error("JSZip not loaded — check CDN script tag");
  const zip=new JSZip();
  const AR=RATIOS[ratio||"16:9"]||RATIOS["16:9"];
  const ratioKey=ratio.replace(":","x");
  const pn=project.name.replace(/[^a-zA-Z0-9]+/g,"_");
  const total=selectedGfx.length;

  const manifest=generatePremiereManifest(project,brand,selectedGfx,ratio,fps);

  for(let i=0;i<selectedGfx.length;i++){
    const g=selectedGfx[i];
    const folderName=manifest.items[i].folder;
    const durSec=g.duration||2;

    if(onProgress) onProgress(i,total,`Rendering ${folderName}…`);

    const frames=await renderPNGSequence(g,brand,ratio,durSec,fps);
    for(let f=0;f<frames.length;f++){
      const frameName=`frame_${String(f+1).padStart(4,"0")}.png`;
      zip.file(`${folderName}/${frameName}`,frames[f]);
    }
  }
  ...
  if(onProgress) onProgress(total,total,"Compressing zip…");
  return zip.generateAsync({type:"blob"},meta=>{
    if(onProgress&&meta.percent) onProgress(total,total,`Compressing… ${Math.round(meta.percent)}%`);
  });
}
...
// Social Media Expert review prompt
  ...
  return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button style={btnDanger({padding:"7px 13px",fontSize:DS.fsSm})} onClick={()=>{setGraphics([]);setGStep("idle");setReviewResults(null);}} title="Clear all and re-analyse">↺ Re-analyse</button>
      </div>
      {/* AI Review Results Panel */}
      {reviewResults&&(
        <div style={card({padding:`${DS.lg}px`,marginBottom:DS.lg,border:"1px solid rgba(255,200,50,0.3)",background:"rgba(255,200,50,0.04)"})}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:DS.md}}>
...
function ExportTab({project,brand,updateProject}){
  ...
  const [premiereExporting,setPremiereExporting]=useState(false);
  ...
  // ── Premiere Package Export ──
  const runPremiereExport=async()=>{
    if(!selectedGfx.length){alert("Select graphics first.");return;}
    setPremiereExporting(true);setPremierePhase("Starting…");
    try{
      for(const ratio of selectedRatios){
        const ratioKey=ratio.replace(":","x");
        const pn=project.name.replace(/\s+/g,"_");
        const blob=await exportPremierePackage(project,brand,selectedGfx,ratio,PREMIERE_FPS,(done,total,msg)=>{
          setPremierePhase(`${ratio} — ${msg} (${done}/${total})`);
        });
        const a=document.createElement("a");a.href=URL.createObjectURL(blob);
        a.download=`${pn}_${ratioKey}_premiere_package.zip`;a.click();
        await new Promise(r=>setTimeout(r,500));
      }
      setPremierePhase("Done!");
    }catch(err){setPremierePhase("Error: "+err.message);console.error(err);}
    setTimeout(()=>setPremiereExporting(false),2000);
  };
  ...
  return(
    <div>
      ...
      {/* Premiere Package Export */}
      {selectedGfx.length>0&&selectedRatios.length>0&&(
        <div style={{marginTop:DS.lg,background:"rgba(100,50,200,0.06)",border:"1px solid rgba(100,50,200,0.2)",borderRadius:12,padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:DS.sm,marginBottom:DS.sm}}>
            <span style={{fontSize:18}}>🎬</span>
            <div style={{flex:1}}>
              <div style={{fontWeight:800,fontSize:13}}>Premiere Pro Pipeline</div>
              <div style={{fontSize:11,color:DS.textMuted,lineHeight:1.4}}>
                PNG sequences + manifest.json — import via Infographic Studio panel
              </div>
            </div>
          </div>
          {premiereExporting
            ?<div style={{fontSize:12,color:DS.textMuted,padding:"8px 0"}}>{premierePhase}</div>
            :<button
              disabled={selectedRatios.length===0||!selectedGfx.length}
              style={{width:"100%",background:"rgba(100,50,200,0.2)",border:"1px solid rgba(100,50,200,0.35)",borderRadius:10,padding:"12px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}
              onClick={runPremiereExport}>
              🎬 Export Premiere Package ({selectedRatios.join(" + ")})
            </button>
          }
          <div style={{fontSize:10,color:DS.textMuted,marginTop:DS.sm,lineHeight:1.5}}>
            Downloads a zip per ratio containing PNG sequence folders and a manifest.json.
            Run the import script in Premiere to auto-place all graphics on the timeline.
          </div>
        </div>
      )}
    </div>
        ...
        <div style={{flex:1,minWidth:0,display:"flex",alignItems:"center",gap:DS.sm}}>
          ...
          {editingName
            ?<input ref={nameRef} value={nameDraft} onChange={e=>setNameDraft(e.target.value)}
              onBlur={()=>{if(nameDraft.trim()&&nameDraft!==project.name){updateProject({name:nameDraft.trim()});const slug=nameDraft.trim().replace(/[^a-zA-Z0-9]+/g,"-").toLowerCase();window.history.replaceState(null,null,"#/project/"+slug);}setEditingName(false);}}
              onKeyDown={e=>{if(e.key==="Enter"){e.target.blur();}if(e.key==="Escape"){setNameDraft(project.name);setEditingName(false);}}}
              style={{fontWeight:700,fontSize:DS.fsLg,background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:DS.rSm,padding:"2px 8px",color:DS.textPrimary,fontFamily:"inherit",outline:"none",minWidth:200}}
              autoFocus/>
            :<div style={{fontWeight:700,fontSize:DS.fsLg,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",cursor:"pointer",borderBottom:"1px dashed rgba(255,255,255,0.2)"}}
              onClick={()=>{setNameDraft(project.name);setEditingName(true);}}
              title="Click to rename project">{project.name}</div>
          }
        </div>
...
function drawTitleCard(canvas, brand, ratio, progress=1){
  ...
  if(style==="bar"){
    ...
    if(B.titleCardSubtitle){
      ...
      drawText(ctx,B.titleCardSubtitle,PAD,H*0.76,W-PAD*2,H*0.12,Math.round(48*sc),"500","left","rgba(255,255,255,0.85)",1,FF);
      ctx.restore();
    }
  }
