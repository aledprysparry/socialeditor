...
// ── API base URL ──
// GitHub Pages → absolute URL to aledparry.com API. Localhost/aledparry.com → relative.
const API_BASE=(typeof window!=="undefined"&&window.location.hostname.includes("github.io"))
  ?"https://www.aledparry.com":"";

// ── CDN / module compat ──
// When running as standalone (Babel in-browser), these globals come from CDN <script> tags.
// When running inside Next.js, they come from npm imports above (re-enable if needed).
// JSZip: loaded via CDN <script> — available as window.JSZip
// docx: loaded via CDN <script> — available as window.docx
// FFmpeg: optional, loaded dynamically at runtime
...
// ═══════════════════════════════════════════════════════════════
// Generate Premiere ExtendScript (.jsx) — auto-imports PNG sequences + places on timeline
function generatePremiereScript(graphics, ratio, prefix, projectName) {
  const FPS = 25;
  ...
  const clips = graphics.map((g, i) => {
    ...
    return { folder, startSec, dur, isOv, label };
  });

  return `// Premiere Pro ExtendScript — run via File > Scripts > Run Script
// ============================================================
// Premiere Pro ExtendScript — Import PNG sequences + place on timeline
// Generated by Social Editor for: ${(projectName||"Project").replace(/"/g,"'")}
// Ratio: ${ratio} (${W}x${H}) @ ${FPS}fps
// ============================================================
//
// HOW TO USE:
// 1. Export "Premiere Ready" from Social Editor
// 2. Unzip the downloaded file into a folder
// 3. Open your Premiere project (or create a new one)
// 4. File > Scripts > Run Script > select this file
// 5. Pick the unzipped folder when prompted
// 6. Script imports each PNG sequence + places on V2/V3
//
// IMPORTANT: Make sure you have an active sequence open before running.
// If not, the script will create one for you.

var FPS = ${FPS};
var clips = ${JSON.stringify(clips, null, 2)};
...
function main() {
  ...
  if (!proj) { alert("No project open. Please open a project first."); return; }

  // Ask user for the unzipped export folder
  var rootFolder = Folder.selectDialog("Select the unzipped Premiere Ready folder");
  if (!rootFolder) return;

  // Get or create active sequence
  var seq = proj.activeSequence;
  if (!seq) {
    // Create a new sequence — user may need to set up manually
    alert("No active sequence. Please create a sequence first (File > New > Sequence), then run this script again.");
    return;
  }

  // Ensure enough video tracks exist (V1 = footage, V2 = fullscreen, V3 = overlays)
  while (seq.videoTracks.numTracks < 4) {
  ...
  for (var i = 0; i < clips.length; i++) {
    var c = clips[i];
    var track = c.isOv ? 2 : 1; // V2 for fullscreen, V3 for overlays
    var startTicks = c.startSec * TICKS_PER_SECOND;

    // Try PNG sequence folder first (animated)
    var seqFolder = new Folder(rootFolder.fsName + "/" + c.folder);
    var firstFrame = new File(seqFolder.fsName + "/frame_0001.png");

    if (seqFolder.exists && firstFrame.exists) {
      // Import as image sequence — suppressUI=true, targetBin=root, asNumberedStills=true
      try {
        // Set image sequence import preference
        app.setSDKEventMessage("Importing: " + c.label, "info");

        // Import the first frame — Premiere auto-detects the sequence
        var importArray = [firstFrame.fsName];
        if (proj.importFiles(importArray, true, proj.rootItem, true)) {
          // Find the just-imported item (last in root bin)
          var item = proj.rootItem.children[proj.rootItem.children.numItems - 1];
          if (item) {
            // Place on timeline at the correct timecode
            seq.videoTracks[track].insertClip(item, startTicks / TICKS_PER_SECOND);
            imported++;
            continue;
          }
        }
      } catch(e) {
        // Fall through to still PNG
      }
    }

    // Fallback: import still PNG
    var stillFile = new File(rootFolder.fsName + "/" + c.folder + ".png");
    if (stillFile.exists) {
      try {
        if (proj.importFiles([stillFile.fsName])) {
          var item2 = proj.rootItem.children[proj.rootItem.children.numItems - 1];
          if (item2) {
            seq.videoTracks[track].insertClip(item2, startTicks / TICKS_PER_SECOND);
            imported++;
            continue;
          }
        }
  ...
  var msg = "Done! Imported " + imported + " of " + clips.length + " graphics.";
  if (failed.length > 0) {
    msg += "\\n\\nFailed (" + failed.length + "):\\n" + failed.join("\\n");
  }
  msg += "\\n\\nFullscreen graphics on V2, Overlays on V3.";
  msg += "\\nYour footage goes on V1.";
  alert(msg);
}
...
function drawGraphic(canvas,g,brand,ratio,progress=1){
  ...
  if(t==="myth"||t==="reality"){
    ...
    // Render as a generic fullscreen card so it's never transparent.
...
// FFmpeg + fetchFile: available via npm import (Next.js) or CDN global (standalone)
const _FFmpeg=typeof FFmpeg!=="undefined"?FFmpeg:(typeof window!=="undefined"&&window.FFmpeg)||null;
const _fetchFile=typeof fetchFile!=="undefined"?fetchFile:(typeof window!=="undefined"&&window.fetchFile)||(async b=>new Uint8Array(await new Response(b).arrayBuffer()));
let _ffmpeg=null;
async function getFFmpeg(){
  if(_ffmpeg&&_ffmpeg.loaded) return _ffmpeg;
  if(!_FFmpeg) throw new Error("FFmpeg not available — load @ffmpeg/ffmpeg from CDN or npm");
  _ffmpeg=new _FFmpeg();
...
async function pngSeqToMov(frames, fps=25, onProgress){
  ...
  for(let i=0;i<frames.length;i++){
    ...
    await ff.writeFile(name, await _fetchFile(frames[i].blob||frames[i]));
  }
  if(onProgress) ff.on("progress",({progress})=>onProgress(progress));
...
function GraphicsTab({project,brand,updateProject,previewRatio}){
  ...
  const runReview=async()=>{
    ...
    try{
      ...
      const raw=await callAI({
        ...
        messages:[{role:"user",content:`VIDEO TITLE: "${project.name}"\n\nTRANSCRIPT:\n${transcript}\n\nGRAPHICS TO REVIEW:\n${graphicsSummary}`}],
        max_tokens:3000
      });
      const parsed=JSON.parse(raw.replace(/```json|```/g,"").trim());
  ...
  const applyReviewSuggestion=(i,suggestions)=>{
    updateGraphicContent(i,suggestions,{});
  };

  const applyAllReviewSuggestions=()=>{
    if(!reviewResults) return;
    reviewResults.forEach(r=>{
      if(r.verdict!=="ok"&&r.verdict!=="remove"&&r.suggestions&&Object.keys(r.suggestions).length){
        applyReviewSuggestion(r.index,r.suggestions);
      }
    });
    setReviewResults(null);
    setTimeout(previewAll,300);
  };
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <button style={{...sm,background:reviewing?"rgba(255,200,50,0.15)":"rgba(255,200,50,0.1)",border:"1px solid rgba(255,200,50,0.35)",opacity:reviewing?0.6:1,cursor:reviewing?"wait":"pointer"}} onClick={()=>!reviewing&&runReview()} title="AI Social Media Expert reviews all slides">
        ...
        <div style={card({padding:`${DS.lg}px`,marginBottom:DS.md,border:"1px solid rgba(255,200,50,0.3)",background:"rgba(255,200,50,0.04)"})}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:DS.md}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button style={btnPositive({fontSize:11,padding:"6px 14px"})} onClick={applyAllReviewSuggestions}>Apply All Suggestions</button>
              <button style={btnIcon()} onClick={()=>setReviewResults(null)} title="Dismiss">✕</button>
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {reviewResults.map((r,ri)=>{
              const verdictColor=r.verdict==="ok"?"rgba(42,157,143,0.8)":r.verdict==="tweak"?"rgba(255,200,50,0.8)":r.verdict==="rewrite"?"rgba(230,57,70,0.8)":"rgba(150,150,150,0.6)";
              const hasSuggestions=r.suggestions&&Object.keys(r.suggestions).length>0;
              return(
                <div key={ri} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${DS.borderSubtle}`,borderRadius:DS.rSm,padding:"10px 14px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:r.issues?.length||hasSuggestions?8:0}}>
                    <span style={{fontWeight:800,fontSize:12,color:verdictColor,textTransform:"uppercase",background:`${verdictColor}22`,padding:"2px 8px",borderRadius:4}}>{r.verdict}</span>
                    <span style={{fontSize:13,fontWeight:600,opacity:0.7}}>Slide #{(r.index||ri)+1} — {graphics[r.index||ri]?.template||"?"}</span>
                    {hasSuggestions&&r.verdict!=="ok"&&(
                      <button style={{...btn({fontSize:10,padding:"3px 10px"}),marginLeft:"auto",background:"rgba(42,157,143,0.15)",border:"1px solid rgba(42,157,143,0.3)"}} onClick={()=>{applyReviewSuggestion(r.index||ri,r.suggestions);doPreview(graphics[r.index||ri],r.index||ri);}}>Apply</button>
                    )}
                  </div>
                  {r.issues?.length>0&&<div style={{fontSize:12,opacity:0.6,marginBottom:hasSuggestions?6:0}}>{r.issues.join(" · ")}</div>}
                  {hasSuggestions&&r.verdict!=="ok"&&(
                    <div style={{fontSize:12,opacity:0.8,fontFamily:"monospace",background:"rgba(0,0,0,0.15)",padding:"6px 10px",borderRadius:4}}>
                      {Object.entries(r.suggestions).map(([k,v])=><div key={k}><span style={{opacity:0.5}}>{k}:</span> {v}</div>)}
                    </div>
                  )}
                </div>
              );
...
function ExportTab({project,brand,updateProject}){
  ...
  const exportCueSheet=async()=>{
    // docx library: available via npm (Next.js) or CDN global window.docx (standalone)
    const dx=typeof window!=="undefined"&&window.docx?window.docx:{Document,Packer,Paragraph,TextRun,Table,TableRow,TableCell,HeadingLevel,AlignmentType,BorderStyle,WidthType,ShadingType};
    const {Document:Doc,Packer:Pk,Paragraph:Para,TextRun:TR,Table:Tbl,TableRow:TblRow,TableCell:TblCell,HeadingLevel:HL,AlignmentType:AT,BorderStyle:BS,WidthType:WT,ShadingType:ST}=dx;
    const border={style:BS.SINGLE,size:1,color:"CCCCCC"};
    const borders={top:border,bottom:border,left:border,right:border};
    const cellMar={top:60,bottom:60,left:100,right:100};
    const hdrShade={fill:"1B4965",type:ST.CLEAR};
    const hdrRun=(t)=>new TR({text:t,bold:true,color:"FFFFFF",font:"Arial",size:18});
    const cellRun=(t,opts={})=>new TR({text:t,font:"Arial",size:18,...opts});

    // Column widths (DXA) — total 9360 (US Letter with 1" margins)
    const colW=[600,1400,900,1600,1200,3660];

    const headerRow=new TblRow({children:["#","Timecode","Duration","Template","Type","Content"].map((h,i)=>
      new TblCell({borders,width:{size:colW[i],type:WT.DXA},shading:hdrShade,margins:cellMar,
        children:[new Para({alignment:AT.LEFT,children:[hdrRun(h)]})]})
    )});

    const gfx=selectedGfx.length?selectedGfx:graphics;
    const dataRows=gfx.map((g,i)=>{
      const meta=TMPL[g.template]||{label:g.template,type:"?"};
      const effectiveType=g.typeOverride||meta.type||"fullscreen";
      const contentStr=Object.entries(g.content||{}).map(([k,v])=>Array.isArray(v)?`${k}: ${v.join(", ")}`:`${k}: ${v}`).join(" | ");
      const vals=[`${i+1}`,g.timestamp||"—",`${g.duration||4}s`,meta.label,effectiveType.toUpperCase(),contentStr];
      const altShade=i%2===0?{fill:"F5F0EB",type:ST.CLEAR}:undefined;
      return new TblRow({children:vals.map((v,ci)=>
        new TblCell({borders,width:{size:colW[ci],type:WT.DXA},shading:altShade,margins:cellMar,
          children:[new Para({children:[cellRun(v)]})]})
      )});
    });
    ...
    const doc=new Doc({
      ...
      sections:[{
        ...
        children:[
          new Para({heading:HL.HEADING_1,spacing:{after:120},children:[new TR({text:project.name||"Graphics Cue Sheet",bold:true,font:"Arial",size:36,color:"1B4965"})]}),
          new Para({spacing:{after:300},children:[cellRun(`Exported ${new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}  •  ${gfx.length} graphics`,{color:"666666",size:18})]}),
          new Tbl({width:{size:9360,type:WT.DXA},columnWidths:colW,rows:[headerRow,...dataRows]}),
          new Para({spacing:{before:400},children:[cellRun(`Generated by Social Editor  •  aledparry.com`,{color:"999999",size:16,italics:true})]}),
        ]
      }]
    });
    const buf=await Pk.toBlob(doc);
...
function App(){
  ...
  useEffect(()=>{
    ...
    brands.forEach(b=>{
      ...
      fetch(`${API_BASE}/api/studio?brand=${b.id}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(light)})
...
// No export default — standalone build mounts via ReactDOM.createRoot
